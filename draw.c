...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  ...
  for(count = 0; count < polygons->lastcol - 2; count+=3){
    draw_line( polygons->m[0][count], polygons->m[1][count],
	       polygons->m[0][count + 1], polygons->m[1][count + 1], s, c);
    draw_line( polygons->m[0][count + 1], polygons->m[1][count + 1],
	       polygons->m[0][count + 2], polygons->m[1][count + 2], s, c);
    draw_line( polygons->m[0][count + 2], polygons->m[1][count + 2],
	       polygons->m[0][count], polygons->m[1][count], s, c);
  }
}
...
void add_box( struct matrix *points,
  ...
  add_polygon(points, x, y, z, x2, y, z, x, y, z2);
  ...
  add_polygon(points, x, y2, z, x2, y2, z, x, y2, z2);
  add_polygon(points, x, y2, z2, x2, y2, z, x2, y2, z2);
  add_polygon(points, x, y, z2, x, y2, z2, x2, y, z2);
  add_polygon(points, x, y2, z2, x2, y2, z2, x2, y , z2);
}
...
void add_sphere( struct matrix * points, 
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    int firstLat = lat * num_steps;
    int nextLat = (firstLat + num_steps) % temp->lastcol;
    for ( longt = longStart; longt < longStop; longt++ ) {
      
      index = firstLat + longt; 
      int indexNext = nextLat + longt;

      add_polygon(points, temp->m[0][index], temp->m[1][index], temp->m[2][index],
		  temp->m[0][indexNext], temp->m[1][indexNext], temp->m[2][indexNext],
		  temp->m[0][indexNext + 1], temp->m[1][indexNext + 1], temp->m[2][indexNext + 1]);
      add_polygon(points, temp->m[0][index], temp->m[1][index], temp->m[2][index],
		  temp->m[0][indexNext + 1], temp->m[1][indexNext + 1], temp->m[2][indexNext + 1],
